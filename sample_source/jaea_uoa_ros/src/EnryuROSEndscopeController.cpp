...
class EnryuROSEndscopeController : public SimpleController
{
public:
    ros::NodeHandle nh;
...
EnryuROSEndscopeController::EnryuROSEndscopeController()
{
...
bool EnryuROSEndscopeController::initialize(SimpleControllerIO* io)
{
    body = io->body();
    dt = io->timeStep();
 ...
 if(io->timeStep() < 0.02){
    ...
    joystickSubscriber = nh.subscribe("joy",1, &EnryuROSEndscopeController::joystickCallback,this);

    return true;
}

bool EnryuROSEndscopeController::start()
{
    return true;
}


bool EnryuROSEndscopeController::initContinuousTracks(SimpleControllerIO* io)
{
...
bool EnryuROSEndscopeController::initPseudoContinuousTracks(SimpleControllerIO* io)
{
    trackL = link("TRACK_L");
    trackR = link("TRACK_R");
    if(!trackL || !trackR){
...
bool EnryuROSEndscopeController::initializeJoints(SimpleControllerIO* io, vector<JointSpec>& specs, const string& prefix)
{
    for(auto& spec : specs){
...
void EnryuROSEndscopeController::initJoystickKeyBind()
{
    operationAxes = {
        {
...
void EnryuROSEndscopeController::controlTracks()
{
    trackL->u() = 0.0;
    trackL->dq() = 0.0;
    trackR->u() = 0.0;
...
bool EnryuROSEndscopeController::control()
{
    {
...
void EnryuROSEndscopeController::setTargetArmPositions()
{
...
void EnryuROSEndscopeController::controlArms()
{
...
void EnryuROSEndscopeController::controlArmsWithPosition()
{
...
void EnryuROSEndscopeController::controlArmsWithVelocity()
{
    for(auto& info : jointInfos){
...
void EnryuROSEndscopeController::controlArmsWithTorque()
{
    for(auto& info : jointInfos){
        auto joint = info.joint;
        double q = joint->q();
...
void EnryuROSEndscopeController::joystickCallback(const sensor_msgs::Joy& msg)
{
    std::lock_guard<std::mutex> lock(mutex);
    latestJoystickState = msg;    
}

CNOID_IMPLEMENT_SIMPLE_CONTROLLER_FACTORY(EnryuROSEndscopeController)
